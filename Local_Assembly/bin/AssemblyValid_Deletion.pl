...
foreach my $p (sort keys %$sv){
      ...
      $indel=parse_age("$opt{project}/$p/alignment.2","$opt{project}/$p/Region.fa","$opt{project}/$p/assembly/contigs.fa", $bp_start, $bp_end); ###indel=0,1,2: 0 mean non indel/sv, 1 mean insertion, 2 mean deletion, 3 mean not sure if exists a SV.
      #my $prefix=basename($opt{gff},".gff");
      $summary{$indel}++;
      print "PARSE:\nindel: $indel\nSeq: $sequence\n";

...
$opt{genome} ||="/rhome/cjinfeng/HEG4_cjinfeng/Variations/SV2ACT/input/HEG4_RAW.refassist.fa.RepeatMasker.masked";
...
foreach my $p (sort keys %$sv){
      ...
      my $subseqlen = $end-$start+1;
      $bp_start= $sv->{$p}->[3]-$flank >= 0 ? $flank : $sv->{$p}->[3];
      $bp_end  = $sv->{$p}->[4]+$flank <= $len->{$sv->{$p}->[0]} ? $subseqlen - $flank + 1 : $subseqlen - ($len->{$sv->{$p}->[0]}-$sv->{$p}->[4]);
      ...
      ####Region sequence
      #my $subseqlen = $end-$start+1;
      my $subseq=substr($seq->{$sv->{$p}->[0]},$start,$subseqlen); 
...
=cut      
      ...
      `/rhome/cjinfeng/software/tools/SVcaller/age_v0.4/src/age_align -indel -both $target $query > $opt{project}/$p/alignment.4`;
      }###step3
      ...
      my ($indel,$sequence)=parse_age("$opt{project}/$p/alignment.4","$opt{project}/$p/Region.fa","$opt{project}/$p/genome/scaffold.fa", $bp_start, $bp_end); ###indel=0,1,2: 0 mean non indel/sv, 1 mean insertion, 2 mean deletion, 3 mean not sure if exists a SV.
      #my $prefix=basename($opt{gff},".gff");
      $summary{$indel}++;
      print "PARSE:\nindel: $indel\nSeq: $sequence\n";
...
###modified for deletion
#Alignment:
# first  seq =>  [   3,999] EXCISED REGION [1337,2000]
# second seq =>  [1851,909] EXCISED REGION [ 908, 245]
sub parse_age
{
my ($align,$target,$query, $bp_start, $bp_end)=@_;
...
while(<IN>){
   ...
   while($block=~/(MATCH.*Alignment time is .*? s)/sg){  ### match every alignment block for each contig
      ...
      if ($match=~/Alignment:\n first\s+seq =>\s+\[\s*(\d+)\,\s*(\d+)\]\n\s+second seq \=\> \s+\[\s*(\d+)\,\s*(\d+)\]\n/){
         print "$1\t$2\t$3\t$4\n";
         if ($1 < $bp_start-100 and $2 > $bp_end+100){ ### perfect alignment cover 200 bp of breakpoint
            $indel=0 unless ($indel == 1); ### indicate no SV exists
         }
      ### excise
      }elsif($match=~/Alignment:\n first\s+seq =>\s+\[\s*(\d+)\,\s*(\d+)\] EXCISED REGION \[\s*(\d+)\,\s*(\d+)\]\n\s+second seq \=\> \s+\[\s*(\d+)\,\s*(\d+)\] EXCISED REGION \[\s*(\d+)\,\s*(\d+)\]\n/){
         print "$1\t$2\t$3\t$4\t$5\t$6\t$7\t$8\n";
         if ($1 < $bp_start-100 and $4 > $bp_end+100){ ### excised alignment cover 200 bp of breakpoint
            if ($2 <= $bp_start+100 and $2 >= $bp_start-100 and $3 <= $bp_end+100 and $3 >= $bp_end-100){ ### excise region cover 100 bp of breakpoint
               print "SV Contig $contig: $6,$7\n";
               if ( abs($6-$7) < 10){
                   $indel = 1
               }
               #$indel=1; ### indicate SV exists
            }elsif(($1 <= $bp_start-100 and $2 >= $bp_end+100) or ($3 <= $bp_start-100 and $4 >= $bp_end+100)){
               $indel=0 unless ($indel == 1); ### indicate no SV exists
            }else{
               $indel=3 unless ($indel == 1 or $indel == 0); ### indicate not sure
            }

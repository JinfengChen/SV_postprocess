...
xx <- barplot(data,beside=TRUE,ylab="Proportion",border=FALSE,ylim=c(0,0.9),col=c("Orange","blue"))
axis(1,c(0.5,max(xx)+0.5),line=0,labels=c("",""))
text(xx[1,]+0.2,rep(-0.08,6),offset=2,labels=x[,2],srt=55,xpd=TRUE)
text(xx[1,],x[,3]/sum(x[,3])+0.03,offset=2,labels=x[,3],srt=55,xpd=TRUE)
text(xx[1,]+1.2,x[,4]/sum(x[,4])+0.03,offset=2,labels=x[,4],srt=55,xpd=TRUE)
legend(max(xx)-3, 0.8,c("Deletion","Insertion"),bty="n",border="NA",lty=c(0,0),cex=1,fill=c("orange","blue"))
#barlegend(11,0.47,0.6,16,"Deletion","Orange")
#barlegend(11,0.44,0.6,16,"Insertion","Blue")
mtext("SV Length (bp)",side=1, at=8,line=5)
